...
- (NSMutableArray *) initializeMenuItems {
  ...
  for (NSString *productID in [SECrackRock sharedInstance].sortedProductIDs) {
    SECrackRockProduct *product = [[SECrackRock sharedInstance].productsByID objectForKey:productID];
    ...
    SECrackRockMenuItem *menuItem =
      [[SECrackRockMenuItem alloc] initWithFrame: CGRectMake(0.0f, 0.0f, self.springboardItemSize.width, self.springboardItemSize.height)
                                 ...
                                 tapHandlerBlock: ^{
                                     __strong SECrackRockViewController *strongSelf = weakSelf;
                                     SECrackRockProduct *blockProduct = [[SECrackRock sharedInstance].productsByID objectForKey:productID];
...
- (void) successfulPurchase:(NSNotification *)notification {
  ...
  NSString *productID = [notification.userInfo objectForKey:SECrackRockUserInfoKey_ProductID];
...
- (void) failedPurchase:(NSNotification *)notification {
  ...
  [[[UIAlertView alloc] initWithTitle: @"Purchase Failed"
                              message: @"There was a transaction error. Please try again later, or contact customer support for assistance."
                             delegate: nil cancelButtonTitle: @"OK" otherButtonTitles: nil] show];
}
...
- (void) incompleteRestore:(NSNotification *)notification {
  ...
  [[[UIAlertView alloc] initWithTitle: @"Restore Issue"
                              message: @"A prior purchase transaction could not be found. To restore the purchased product, tap the product's icon. Paid customers will NOT be charged again, but the purchase will be restored."
                             delegate: nil cancelButtonTitle: @"OK" otherButtonTitles: nil] show];
}
